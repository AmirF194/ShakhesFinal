...
public class RefahListActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_refah_list);
        ...
        final String[][] finalData_dorm = data_refah;
        for (int i = 0; i < 10; i++) {
            final int finalI = i;
            layouts[i].setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    Intent myIntent = new Intent(RefahListActivity.this, SingleNewsActivity.class);
        ...
        for (int i = 0; i < 10; i++) {
            ...
            titles[i].setText(convertNums(data_refah[i][2]));
            abstracs[i].setText(convertNums(data_refah[i][3]));
            calendar = new JalaliCalendar(getDateTimeFromTimestamp(Long.parseLong(data_refah[i][1])));
